...
JIRA is our issue tracking tool. It is a responsibility of the entire team to
keep it updated :wink:
...
- Gitflow:Create a new branch from `develop` branch, remember to pull for latest
  changes. Name it as the ticket key, with prefix **feat/fix/bug/docs**,
  depending on the ticket's type. Example: `feat/ABC-1234`.
- Trunk-based development: All developers work in the same **master** branch. No
  need to create new branches or make pull-request (unless breaking changes are
  introduced).
